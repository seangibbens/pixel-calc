  ...
  return (
    ...
    >
      <svg
        ...
        height="24"
        viewBox="0 0 24 24"
        style={{ fill: "#3282b8", marginLeft: "15px" }}
      >
        <path
          fill-rule="evenodd"
...
const LinkedinLink = () => {
  return (
    <a
      href="https://www.linkedin.com/in/sean-gibbens/"
      target="_blank"
      rel="noopener noreferrer"
    >
      <svg
...
const Calc = () => {
  const [ogHeight, setOgHeight] = useState("");
  const [ogWidth, setOgWidth] = useState("");
  const [percentage, setPercentage] = useState("");
  const [newHeight, setNewHeight] = useState("");
  const [newWidth, setNewWidth] = useState("");

  const handleOgHeightChange = (e) => {
    setOgHeight(e.target.value);
  };
  const handleOgWidthChange = (e) => {
    setOgWidth(e.target.value);
  };
  const handlePercentageChange = (e) => {
    setPercentage(e.target.value);
  };

  const onScale = (percent) => {
    setPercentage(percent);
  };

  const onCalculate = () => {
    const nh = ogHeight * (percentage / 100);
    const nw = ogWidth * (percentage / 100);
    setNewHeight(nh);
    setNewWidth(nw);
  };
  ...
  return (
    <div className="calc">
      <div className="col-1">
        ...
        <input type="number" value={ogHeight} onChange={handleOgHeightChange} />
        <div className="input-label">
          <span>Width </span>
          <i>(in px)</i>
        </div>
        <input type="number" value={ogWidth} onChange={handleOgWidthChange} />
      </div>
      ...
      <div className="col-5">
        ...
        <input type="number" value={newHeight} readOnly />
        <div className="input-label">
          <span>New Width </span>
          <i>(in px)</i>
        </div>
  ...
  return (
    <div className="app">
      <div className="title">
        ...
        <i>a tool for quickly calculating pixel scale ups or downs</i>
      </div>
      <Calc />
      <div className="footer">
        <span>Created by Sean Gibbens</span>
        <GithubLink />
        <LinkedinLink />
      </div>
    </div>
